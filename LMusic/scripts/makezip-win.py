...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\LMusic Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\LMusic Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\LMusic manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\LMusic.vst3\\Contents\\x86_64-win\\LMusic.vst3",
      projectpath + "\\build-win\\LMusic_x64.exe"  
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\LMusic-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\LMusic-app_x64.pdb"  
  ]
