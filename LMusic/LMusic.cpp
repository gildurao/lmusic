    ...
    : Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  ...
  mLayoutFunc = [&](IGraphics *pGraphics)
  {
    ...
    pGraphics->AttachControl(new IVButtonControl(keyboardBounds.GetFromTRHC(200, 30).GetTranslated(0, -30), SplashClickActionFunc,
                                                 "Show/Hide Keyboard", DEFAULT_STYLE.WithColor(kFG, COLOR_WHITE).WithLabelText({15.f, EVAlign::Middle})))
    ...
    if (!IsOOPAuv3AppExtension())
    {
      pGraphics->AttachControl(new IVButtonControl(
          b.GetFromTRHC(100, 100), [pGraphics](IControl *pCaller)
          {
...
#if IPLUG_DSP
void LMusic::ProcessBlock(sample **inputs, sample **outputs, int nFrames)
{
...
void LMusic::ProcessMidiMsg(const IMidiMsg &msg)
{
  TRACE;
...
bool LMusic::OnMessage(int msgTag, int ctrlTag, int dataSize, const void *pData)
{
